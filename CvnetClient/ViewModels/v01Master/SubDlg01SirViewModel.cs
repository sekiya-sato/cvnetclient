...
namespace CvnetClient.ViewModels
{
    public partial class SubDlg01SirViewModel : BaseViewModel
    {
        ...
        string? startCode;
        [ObservableProperty]
        private bool isAutoOrderVisible;
        ...
        [ObservableProperty]
        private Dictionary<int, string>? comboClosingDay;

        // 締日2 (closingDay2) options
        [ObservableProperty]
        private Dictionary<int, string>? comboClosingDay2;

        // 締日3 (closingDay3) options
        [ObservableProperty]
        private Dictionary<int, string>? comboClosingDay3;
        ...
        [ObservableProperty]
        private Dictionary<int, string>? comboScheduledPaymentDay;

        // 支払予定日2 (scheduledPaymentDay2) options
        [ObservableProperty]
        private Dictionary<int, string>? comboScheduledPaymentDay2;

        // 支払予定日3 (scheduledPaymentDay3) options
        [ObservableProperty]
        private Dictionary<int, string>? comboScheduledPaymentDay3;
        ...
        private Dictionary<int, string> GenerateScheduledPaymentDaysDict()
        {
            ...
            for (int i = 1; i <= 28; i++)
                d[i] = $"{i:D2}";
            d[99] = "99 末日"; // end of month
            return d;
        }

        // ✅ Generate scheduled payment month dictionary (0–6)
        private Dictionary<int, string> GenerateScheduledPaymentMonths => new Dictionary<int, string>
            {
        ...
        public bool GetIsAutoOrderVisible()
        {
        ...
        void Init()
        {
            ...
            ComboClosingDay = GenerateScheduledPaymentDaysDict();
            EditSupplier.ClosingDay = ComboClosingDay.First().Key;

            ComboClosingDay2 = GenerateScheduledPaymentDaysDict();
            EditSupplier.ClosingDay2 = ComboClosingDay2.First().Key;

            ComboClosingDay3 = GenerateScheduledPaymentDaysDict();
            EditSupplier.ClosingDay3 = ComboClosingDay3.First().Key;
            ...
            ComboScheduledPaymentDay = GenerateScheduledPaymentDaysDict();
            EditSupplier.ScheduledPaymentDay = ComboScheduledPaymentDay.Last().Key;

            ComboScheduledPaymentDay2 = GenerateScheduledPaymentDaysDict();
            EditSupplier.ScheduledPaymentDay2 = ComboScheduledPaymentDay2.Last().Key;

            ComboScheduledPaymentDay3 = GenerateScheduledPaymentDaysDict();
            EditSupplier.ScheduledPaymentDay3 = ComboScheduledPaymentDay3.Last().Key;

            // ✅ 支払先 (Payee)
            var comboList2 = new Dictionary<string, string>();

            string sql_paymentCombo = @"
                    SELECT 仕入先CD, 仕入先名
                    FROM HC$Master_SIIRE
                    WHERE 発注停止FLG = 0
                    ORDER BY 仕入先CD";

            var get_paymentCombo = AppData.Http?.AspxSqlQuery(sql_paymentCombo, null);
            foreach (DataRow row in get_paymentCombo.Rows)
            {
                string? key = row[0] != DBNull.Value ? row[0].ToString() : string.Empty;
                string? value = row[1] != DBNull.Value ? row[1].ToString() : string.Empty;
                comboList2.Add(key, $"{key} {value}");
            }
        ...
        void SubList(string startCd, string sql_P1, string sql_P2, int sql_P3)
        {
            ...
            var list = (from DataRow dr in retData.Rows
                        select new MasterSupplier
                        {
                            ...
                            TaxCd = Convert.ToDecimal(dr["消費税CD"]),
